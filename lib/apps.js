...
Apps.prototype = {
  ...
  lookupOrgFromSpace: function* (space) {
    try {
      let space_response = yield this.heroku.request({path: `/spaces/${space}`});
      console.log(`${space_response.organization.name}`);
      return yield this.heroku.request({path: `/organizations/${space_response.organization.name}`});
    } catch (err) {
      if (err.statusCode === 404) {
        return null;
      } else {
  ...
  createNewApp: function* (oldApp, newAppName, region, space) {
    ...
    let org = null;
    if (space){
      org = yield this.lookupOrgFromSpace(space);
    }
    else{
      org = yield this.lookupOrgFromApp(oldApp);
    }
    let info = {
      name: newAppName,

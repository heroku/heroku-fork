...
Addons.prototype = {
  addAddon: function* (app, addon, attachments) {
    ...
    let request = this.heroku.apps(app.name).addons().create({attachment: {name: attachment.name}, plan: addon.plan.name}).catch(catchShutdown);
  ...
  attachAddon: function* (app, addon, attachments) {
    for (let attachment of attachments) {
      let request = this.heroku.addonAttachments().create({addon: addon.id, app: app.id, name: attachment.name});
